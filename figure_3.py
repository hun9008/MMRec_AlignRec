...
# === 수동 텍스트 (직접 정의한 요약 문장) ===
COMMON_ITEM_TEXT = (
    "JJ Cole Bundleme ... This item features with warm inner shearling and "
    "weather-resistant outer shell ..."
)

ALIGNREC_TEXTS = [
    "JJ Cole Bundleme ... It is perfect for mild-moderately cold weather and "
    "is wind and water ...",
    "JJ Cole Original Infant Bundleme ... With a plush inner faux shearling "
    "and luxuriously soft outer thermaplush cover ... ",
    "JJ Cole Urban Bundleme ... Bundleme protects against wind and rain while "
    "the luxuriously soft inner thermaplush ... ",
]

ANCHOR_TEXTS = [
    "JJ Cole Bundleme ... It is perfect for mild-moderately cold weather and "
    "is wind and water ...",
    "JJ Cole Arctic Bundleme ... It provides oremium protection and comfort "
    "for vour baby in extreme weather conditions ...",
    "JJ Cole Original Infant Bundleme ... With a plush inner faux shearling "
    "and luxuriously soft outer thermaplush cover ... ",
]
...
def visualize_alignrec_vs_anchor(
    ...
    anchor_text_override: Optional[str] = None,
    alignrec_texts_override: Optional[List[str]] = None,
    anchor_texts_override: Optional[List[str]] = None,
):
    with tick("시각화 생성"):
        ...
        ax_a.axis("off")
        # meta에서는 이미지만 사용, 텍스트는 override 사용
        _, _, img_a = get_item_text_and_img(anchor_i, id2asin, meta)
        ...
        # anchor text
        anchor_text = anchor_text_override if anchor_text_override else ""
        if anchor_text:
            ax_a.text(
                0.02,
                -0.20,
                wrap_text(anchor_text, width=40, max_lines=8),
                transform=ax_a.transAxes,
                va="top",
                ha="left",
                fontsize=10,
        ...
        for idx in range(topk):
            ...
            asin, _, img = get_item_text_and_img(item_id, id2asin, meta)
            ...
            # 여기서 meta text 대신 override 텍스트 사용
            if alignrec_texts_override is not None and idx < len(alignrec_texts_override):
                text_to_show = alignrec_texts_override[idx]
            else:
                text_to_show = ""
            ...
            if text_to_show:
                ax.text(
                    0.02,
                    TEXT_Y,
                    wrap_text(text_to_show, width=35, max_lines=6),
                    transform=ax.transAxes,
                    va="top",
                    ha="left",
                    fontsize=10,
        ...
        for idx in range(topk):
            ...
            # 여기서도 meta text 대신 override 텍스트 사용
            if anchor_texts_override is not None and idx < len(anchor_texts_override):
                text_to_show = anchor_texts_override[idx]
            else:
                text_to_show = ""
...
def main():
    ...
    # txt cos (override 텍스트는 시각화용이라, 여기서는 기존 로직 유지)
    txt_sims = None
    ...
    visualize_alignrec_vs_anchor(
        ...
        img_sims=img_sims,
        txt_sims=txt_sims,
        anchor_text_override=COMMON_ITEM_TEXT,
        alignrec_texts_override=ALIGNREC_TEXTS,
        anchor_texts_override=ANCHOR_TEXTS,
    )
