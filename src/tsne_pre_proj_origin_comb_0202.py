...
def _extract_origin_proj_embeddings(model, norm_adj, device):
    with torch.no_grad():
...
def _extract_pretrain_embeddings(model):
    ...
    return {"ID": id_init, "MM": mm_pre, "Text": t_pre, "Vision": v_pre}


def _tsne_2d_combined(pretrain, origin, proj, indices, random_state=42):
    keys = ["ID", "MM", "Text", "Vision"]
    arrays = []
    labels = []

    for k in keys:
        arrays.append(pretrain[k].detach().cpu().numpy()[indices])
        labels.append(f"Pre-{k}")
    for k in keys:
...
def visualize_pre_proj_origin_combined(pretrain, origin, proj, sample_size, title, filename, seed=42):
    ...
    tsne_2d = _tsne_2d_combined(pretrain, origin, proj, indices, random_state=seed)
    ...
    ax.set_title("Pretrain + Origin + Projection (single t-SNE)")
...
def main(args):
    ...
    # Pretrain snapshot before loading checkpoint
    ...
    origin, proj = _extract_origin_proj_embeddings(model, model.norm_adj, config["device"])

    visualize_pre_proj_origin_combined(
        pretrain,
        origin,
        proj,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--title",
        type=str,
        default="t-SNE: pretrain + origin + projection (ID/MM/Text/Vision)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="tsne_pre_proj_origin_comb_0203.png",
    )
