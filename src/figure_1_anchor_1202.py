...
def visualize_alignment_3views(
    ...
    위: 전체 뷰, 아래: 위에서 지정한 원형 영역을 확대해서 표시.
    """
    # ===== 1. tensor -> numpy =====
    id_np = id_embeds.detach().cpu().numpy()
    content_np = content_embeds_items.detach().cpu().numpy()
    ...
    # ===== 5. zoom 위치: final_2d 가 가장 몰려있는 클러스터의 중심 근처 =====
    ...
    # 자동 center 대신 수동 고정값을 쓰고 싶어서 아래처럼 override
    ...
    fig, (ax_main, ax_zoom) = plt.subplots(
        2, 1,
        figsize=(10, 8),
        gridspec_kw={'height_ratios': [1.2, 2.0], 'hspace': 0.1}
    )
    ...
    scat1 = ax_zoom.scatter(id_2d[:, 0], id_2d[:, 1], alpha=0.7, marker='o', s=64)
    scat2 = ax_zoom.scatter(content_2d[:, 0], content_2d[:, 1], alpha=0.7, marker='s', s=64)
    scat3 = ax_zoom.scatter(final_2d[:, 0], final_2d[:, 1], alpha=0.9, marker='*', s=88)
...
def run_tsne_visualization(model_name, checkpoint_path, lines_to_draw):
    ...
    # ----- 3) t-SNE 그리기 -----
    filename = f"tsne_{model_name}_3view_top{lines_to_draw}_anchor_1201.png"
