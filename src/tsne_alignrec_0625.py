...
def main(args):
    ...
    with torch.no_grad():
        # train=True로 설정해서 mm_embeds도 받도록
        _, _, mm_embeds, content_embeds = model.forward(norm_adj, train=True)

        print("content_embeds.shape[0]:", content_embeds.shape[0])
        print("mm_embeds.shape[0]:", mm_embeds.shape[0])
        print("n_users:", n_users)
        print("n_items:", n_items)

        # 마지막 n_items만 추출 (아이템 임베딩)
        content_embeds_items = content_embeds[-n_items:]
        mm_embeds_items = mm_embeds[-n_items:]

    # 시각화 (ID vs multimodal)
    visualize_alignment(content_embeds_items, mm_embeds_items)
