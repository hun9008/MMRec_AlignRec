...
def run_elbow_evaluation(
    ...
    config_dict = {
        'multimodal_data_dir': f'data/{dataset_name}_beit3_128token_add_title_brand_to_text/',
        'save_model': False,
        'side_emb_div': 2,
    ...
    if os.path.exists(emb_cache_path):
        ...
        with torch.no_grad():
            forward_outputs = model.forward(norm_adj, train=True)
            if len(forward_outputs) == 4:
                all_users, all_items, mm_embeds, content_embeds = forward_outputs
            elif len(forward_outputs) == 6:
                all_users, all_items, mm_embeds, content_embeds, t_emb, v_emb = forward_outputs
            else:
                raise ValueError(f"Unexpected forward output count: {len(forward_outputs)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--output_csv',
        type=str,
        default='cache_tsne/k_means_elbow_sports_0604.csv',
        help='CSV path to save k/inertia results. Use an empty string to skip.',
    )
    parser.add_argument(
        '--output_plot',
        type=str,
        default='cache_tsne/k_means_elbow_sports_0604.png',
        help='PNG path to save elbow plot. Use an empty string to skip.',
    )
    args = parser.parse_args()

    models = [
        ("ALIGNREC_STORE", "saved/ALIGNREC_STORE_sports_best.pth"),
    ]
