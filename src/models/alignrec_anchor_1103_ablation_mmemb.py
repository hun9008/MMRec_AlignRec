...
class ALIGNREC_ANCHOR_1103_ABLATION_MMEMB(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_1103_ABLATION_MMEMB, self).__init__(config, dataset)
        self.sparse = True
        ...
        # self.p_mm = nn.Sequential(nn.Linear(self.embedding_dim, self.embedding_dim),
        #                         nn.ReLU(),
        #                         nn.Linear(self.embedding_dim, self.embedding_dim))
        # self.p_t  = nn.Sequential(nn.Linear(self.embedding_dim, self.embedding_dim),
        #                         nn.ReLU(),
        #                         nn.Linear(self.embedding_dim, self.embedding_dim))
        # self.p_v  = nn.Sequential(nn.Linear(self.embedding_dim, self.embedding_dim),
        #                         nn.ReLU(),
        #                         nn.Linear(self.embedding_dim, self.embedding_dim))

        # # init
        # for block in [self.p_mm, self.p_t, self.p_v]:
        #     for layer in block:
        ...
        # init
        for block in [self.W_id_i, self.W_v, self.W_t, self.D_mm2id]:
            for layer in block:
                if isinstance(layer, nn.Linear):
                    nn.init.xavier_uniform_(layer.weight)
    ...
    def forward(self, adj, users=None, train=False):
        ...
        if train:
            return final_user_e, final_item_e, content_embeds, t_embeds, v_embeds
    ...
    def _neg_cosine(self, x, y):
        # 평균 negative cosine similarity (BM3는 cosine 일치 유도)
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        ua_embeddings, ia_embeddings, content_embeds, t_embeds, v_embeds = \
        ...
        # mixup_mm_fusion_1 = h_mm_i_fusion + h_v_fusion * 0.25 + h_t_fusion * 0.75
        # mixup_mm_fusion_2 = h_mm_i_fusion + h_v_fusion * 0.5 + h_t_fusion * 0.5
        # mixup_mm_fusion_3 = h_mm_i_fusion + h_v_fusion * 0.75 + h_t_fusion * 0.25

        # mixup_mm_t_fusion = h_mm_i_fusion * 0.7 + h_t_fusion * 0.3
        # mixup_mm_v_fusion = h_mm_i_fusion * 0.7 + h_v_fusion * 0.3
        ...
        ii_sim_loss = 0.03 * ii_t_sim_loss + 0.01 * ii_v_sim_loss
        ...
        # )
        cl_v = self.InfoNCE(
            h_id_i_fusion[pos_items].detach(),
        ...
        # cl_mixup_t = self.InfoNCE(
        #     h_mm_i_fusion[pos_items].detach(),
        #     mixup_mm_t_fusion[pos_items],
        #     0.2
        # )
        # cl_mixup_v = self.InfoNCE(
        #     h_mm_i_fusion[pos_items].detach(),
        #     mixup_mm_v_fusion[pos_items],
        #     0.2
        # )
        # cl_mixup_mm_1 = self.InfoNCE(
        #     h_mm_i_fusion[pos_items].detach(),
        #     mixup_mm_fusion_1[pos_items],
        #     0.2
        # )
        # cl_mixup_mm_2 = self.InfoNCE(
        #     h_mm_i_fusion[pos_items].detach(),
        #     mixup_mm_fusion_2[pos_items],
        #     0.2
        # )
        # cl_mixup_mm_3 = self.InfoNCE(
        #     h_mm_i_fusion[pos_items].detach(),
        #     mixup_mm_fusion_3[pos_items],
        #     0.2
        # )

        # cl_origin = self.InfoNCE(
        #     content_embeds_items[pos_items],
        #     side_embeds_items[pos_items],
        #     0.2
        # )
        # cl_origin_user = self.InfoNCE(
        #     content_embeds_user[users],
        #     side_embeds_users[users],
        #     0.2
        # )
        ...
        # row_idx, col_idx = self.mm_original_adj.indices()  # pseudo: dense에서라면 .nonzero()
        ...
        # ====================================================
        # mm_mask_loss = self._intra_mask_loss_identity(h_mm_i_fusion[pos_items])
        # t_mask_loss  = self._intra_mask_loss_identity(h_t_fusion[pos_items])
        # v_mask_loss  = self._intra_mask_loss_identity(h_v_fusion[pos_items])

        # intra_mask_loss = mm_mask_loss
