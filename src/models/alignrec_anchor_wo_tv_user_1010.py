...
class ALIGNREC_ANCHOR_WO_TV_USER_1010(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_WO_TV_USER_1010, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, users=None, train=False):
        ...
        mm_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_v(mm_feats))  # h_enc^i
        ...
        content_embeds = all_embeddings  # [h_id^u ; h_id^i]

        # ----------------------
        # Item-Item Views (MM/T/V)
        # ----------------------
        # MM graph
        if self.sparse:
        ...
        # Project MM to users via R (이건 유지: side_embeds_user를 위해 필요)
        mm_user_embeds = torch.sparse.mm(self.R, mm_item_embeds)
        mm_embeds = torch.cat([mm_user_embeds, mm_item_embeds], dim=0)  # [h_mm^u ; h_mm^i]

        # Text graph (아이템만 사용)
        if t_item_embeds is not None:
            if self.sparse:
                for _ in range(self.n_layers):
        ...
        # Vision graph (아이템만 사용)
        if v_item_embeds is not None:
            if self.sparse:
                for _ in range(self.n_layers):
        ...
        #   item:   content + mm + t + v
        # ----------------------
        content_user, content_item = torch.split(content_embeds, [self.n_users, self.n_items], dim=0)
        mm_user, mm_item = torch.split(mm_embeds, [self.n_users, self.n_items], dim=0)

        # t/v를 user에는 포함하지 않음
        final_users = content_user + mm_user

        # item에는 t/v 포함
        add_items = content_item + mm_item
        if t_item_embeds is not None:
            add_items = add_items + t_item_embeds
        if v_item_embeds is not None:
            add_items = add_items + v_item_embeds
        final_items = add_items

        # side_emb_div 적용(있다면)
        if self.side_emb_div != 0:
            final_users = final_users / self.side_emb_div
            final_items = final_items / self.side_emb_div

        all_embeddings_users = final_users
        all_embeddings_items = final_items

        # ----------------------
        # 호환성 유지용 반환 텐서 (t/v는 user=0, item=값)
        # ----------------------
        zeros_users = torch.zeros((self.n_users, self.embedding_dim), device=self.device)
        if t_item_embeds is not None:
            t_embeds = torch.cat([zeros_users, t_item_embeds], dim=0)
        else:
            t_embeds = torch.cat([zeros_users, torch.zeros((self.n_items, self.embedding_dim), device=self.device)], dim=0)

        if v_item_embeds is not None:
            v_embeds = torch.cat([zeros_users, v_item_embeds], dim=0)
        else:
            v_embeds = torch.cat([zeros_users, torch.zeros((self.n_items, self.embedding_dim), device=self.device)], dim=0)

        if self.use_hist_decoder and train:
            hist_seq = mm_item_embeds[self.user_topk_hist[users], :]
            hist_seq = torch.where(torch.unsqueeze(self.user_topk_hist[users], dim=-1) == -1,
                                torch.zeros_like(hist_seq), hist_seq)
            ...
            return all_embeddings_users, all_embeddings_items, mm_embeds, content_embeds, t_embeds, v_embeds, hist_hid[:, -1, :]

        # train 경로
        if train:
            return all_embeddings_users, all_embeddings_items, mm_embeds, content_embeds, t_embeds, v_embeds
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        if self.use_hist_decoder:
            ua_embeddings, ia_embeddings, side_embeds, content_embeds, t_embeds, v_embeds, user_hist_seq = self.forward(
                self.norm_adj, users, train=True
            )
        else:
            ua_embeddings, ia_embeddings, side_embeds, content_embeds, t_embeds, v_embeds = self.forward(
                self.norm_adj, users, train=True
            )
        ...
        cl_v  = self.InfoNCE(h_mm_i_fusion[pos_items].detach(), h_v_fusion[pos_items], 0.2)
        cl_t  = self.InfoNCE(h_mm_i_fusion[pos_items].detach(), h_t_fusion[pos_items], 0.2)
    ...
    def full_sort_predict(self, interaction):
        user = interaction[0]
        (restore_user_e, restore_item_e,
        mm_embeds, content_embeds, t_embeds, v_embeds) = self.forward(self.norm_adj)

        save_dir = "saved_emb/epoch_last"  # 필요시 config로 받기 (예: config['save_emb_dir'])
        os.makedirs(save_dir, exist_ok=True)

        # ---------- Alignment 미반영 (RAW) ----------
        # raw id: LightGCN(UI-view) item 파트 (projection/fusion 전)
        h_id_raw = content_embeds[self.n_users:]                # (n_items, d)
        np.save(os.path.join(save_dir, "item_emb_raw_id.npy"),
                h_id_raw.detach().cpu().numpy())

        # 원본 text/vision feature (투영/게이트/전파 전)  ※ dtype 통일(=float32) 권장
        np.save(os.path.join(save_dir, "item_feat_raw_text.npy"),
                self.t_feat.detach().cpu().float().numpy())
        np.save(os.path.join(save_dir, "item_feat_raw_vision.npy"),
                self.v_feat.detach().cpu().float().numpy())

        # ---------- Alignment 반영 ----------
        # 최종 아이템 임베딩 (추천에 실제 사용)
        np.save(os.path.join(save_dir, "item_emb_final_alignrec.npy"),
                restore_item_e.detach().cpu().numpy())

        # (alignment space 밖) 멀티모달: projection 전 mm item 파트
        h_mm_out = mm_embeds[self.n_users:]
        np.save(os.path.join(save_dir, "item_emb_mm_out.npy"),
                h_mm_out.detach().cpu().numpy())

        # (alignment space 안) id / mm / text / vision (projection 후)
        with torch.no_grad():
            h_id_align = self.W_id_i(h_id_raw)
            h_mm_align = self.W_mm_i(h_mm_out)
            h_t_align  = self.W_t(self.t_feat)   # 원피처를 align space로
            h_v_align  = self.W_v(self.v_feat)

            np.save(os.path.join(save_dir, "item_emb_align_id.npy"),
                    h_id_align.cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_align_mm.npy"),
                    h_mm_align.cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_align_text.npy"),
                    h_t_align.cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_align_vision.npy"),
                    h_v_align.cpu().numpy())

            # 평균 projection (id_align + mm_align)/2
            h_proj_avg = (h_id_align + h_mm_align) / 2
            np.save(os.path.join(save_dir, "item_emb_align_projavg.npy"),
                    h_proj_avg.cpu().numpy())

        # (선택) 그래프 전파가 반영된 t/v 임베딩도 보관하고 싶으면:
        # np.save(os.path.join(save_dir, "item_emb_tv_graph_text.npy"),
        #         t_embeds[self.n_users:].detach().cpu().numpy())
        # np.save(os.path.join(save_dir, "item_emb_tv_graph_vision.npy"),
        #         v_embeds[self.n_users:].detach().cpu().numpy())
