...
class SMORE_STORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SMORE_STORE, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, train=False):
        ...
        _, id_item = torch.split(content_embeds, [self.n_users, self.n_items], dim=0)
        _, mm_item = torch.split(side_embeds, [self.n_users, self.n_items], dim=0)

        if train:
            return all_embeddings_users, all_embeddings_items, side_embeds, content_embeds

        return all_embeddings_users, all_embeddings_items, id_item, mm_item
    ...
    def full_sort_predict(self, interaction):
        ...
        restore_user_e, restore_item_e, id_item, mm_item = self.forward(self.norm_adj)
        u_embeddings = restore_user_e[user]

        save_dir = os.path.join("saved_emb", self.dataset_name, "SMORE")
