...
class ALIGNREC_SINGLE_0504(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_SINGLE_0504, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, users=None, train=False):
        ...
        if self.side_emb_div!=0:
            all_embeds = content_embeds + t_embeds + v_embeds/self.side_emb_div
        else: 
            all_embeds = content_embeds + t_embeds + v_embeds
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        v_feat_items = self.v_feat[pos_items]  # (batch_size, dim)
        t_feat_items = self.t_feat[pos_items]
        ...
        cl_loss_v_id = self.InfoNCE(side_t_embeds_items[pos_items], content_embeds_items[pos_items], 0.2)
        cl_loss_t_id = self.InfoNCE(side_v_embeds_items[pos_items], content_embeds_items[pos_items], 0.2)
        cl_loss_v_t  = self.InfoNCE(side_v_embeds_items[pos_items], side_t_embeds_items[pos_items], 0.2)
