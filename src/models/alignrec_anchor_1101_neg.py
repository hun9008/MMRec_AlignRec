...
class ALIGNREC_ANCHOR_1101_NEG(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_1101_NEG, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, users=None, train=False):
        ...
        if self.use_recovered_mm_for_final:
            ...
            h_id_i = self.W_id_i(content_embeds_items)
            h_mm_i = self.W_mm_i(mm_embeds_items)

            final_user_e, final_item_e = torch.split(all_embeds, [self.n_users, self.n_items], dim=0)
            final_item_e = h_id_i + h_mm_i + self.recover_lambda * mm_rec_i
        else:
