...
class ALIGNREC_INPUT_CL_ANCHOR_MMTV_REC_OVERLAP_0915(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_INPUT_CL_ANCHOR_MMTV_REC_OVERLAP_0915, self).__init__(config, dataset)
        self.sparse = True
    ...
    def full_sort_predict(self, interaction, K_list=(20,), exclude_seen=False):
        """
        저장물(추천 관점):
        - user_emb_final.npy                     : (n_users, d)
        - item_emb_final_alignrec.npy           : (n_items, d)
        - item_emb_id_items.npy / mm / t / v    : (n_items, d)
        - item_emb_id_plus_mm.npy               : (n_items, d)
        - rec_top{K}_{alias}.npy                : (n_users, K)
            alias in {final, id, mm, text, vision, idmm}
        """
        user = interaction[0]
        save_dir = "saved_emb/rec_output"
        os.makedirs(save_dir, exist_ok=True)

        with torch.no_grad():
            # forward: (users, items, mm_all, id_all, t_all, v_all)
            U_all, V_all, mm_all, id_all, t_all, v_all = self.forward(self.norm_adj)

            # ---- 0) 유저/아이템 분리 ----
            U = U_all                  # (n_users, d)
            V_final = V_all            # (n_items, d)  # 주의: 이미 아이템 파트임

            id_items = id_all[self.n_users:]     # (n_items, d)
            mm_items = mm_all[self.n_users:]
            t_items  = t_all[self.n_users:]
            v_items  = v_all[self.n_users:]

            # ---- 1) 임베딩 저장 ----
            np.save(os.path.join(save_dir, "user_emb_final.npy"),            U.detach().cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_final_alignrec.npy"),   V_final.detach().cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_id_items.npy"),         id_items.detach().cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_mm_items.npy"),         mm_items.detach().cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_t_items.npy"),          t_items.detach().cpu().numpy())
            np.save(os.path.join(save_dir, "item_emb_v_items.npy"),          v_items.detach().cpu().numpy())

            # 조합(예: id + mm) — 필요 조합 더 추가 가능
            idmm_items = id_items + mm_items
            np.save(os.path.join(save_dir, "item_emb_id_plus_mm.npy"),       idmm_items.detach().cpu().numpy())

            # ---- 2) (선택) 본 아이템 제외 마스크 생성 ----
            # exclude_seen=True 이면 학습 상 본 아이템(implicit feedback) 제외
            seen_mask = None
            if exclude_seen:
                # self.interaction_matrix: scipy coo (n_users x n_items)
                coo = self.interaction_matrix.tocoo()
                # torch에서 (n_users, n_items) bool mask 생성 (메모리 신경 쓰면 csr->배치 계산로 바꿔도 됨)
                seen_mask = torch.zeros((self.n_users, self.n_items), dtype=torch.bool)
                seen_mask[torch.from_numpy(coo.row), torch.from_numpy(coo.col)] = True

            # ---- 3) 유저별 Top-K 추천 추출 & 저장 ----
            def topk_rec(U, V, K, seen_mask=None):
                Uc = U.detach().cpu()
                Vc = V.detach().cpu()
                S = torch.matmul(Uc, Vc.t())  # (n_users, n_items)
                if seen_mask is not None:
                    # seen_mask: True인 (u,i)를 -inf
                    S[seen_mask] = -float("inf")
                return torch.topk(S, k=K, dim=1, largest=True).indices.numpy()
            ...
            for K in K_list:
                for name, Vcand in aliases.items():
                    rec = topk_rec(U, Vcand, K, seen_mask=seen_mask)
                    np.save(os.path.join(save_dir, f"rec_top{K}_{name}.npy"), rec)

            # ---- 기존 반환 (평가 파이프 호환) ----
            u_embeddings = U[user]
            scores = torch.matmul(u_embeddings, V_final.transpose(0, 1))
            return scores
