...
class ANCHORREC_ABLATION_SINGLE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ANCHORREC_ABLATION_SINGLE, self).__init__(config, dataset)
        self.sparse = True
        ...
        # init
        for block in [self.W_id_i, self.W_mm_i, self.D_mm2id]:
            for layer in block:
                if isinstance(layer, nn.Linear):
                    nn.init.xavier_uniform_(layer.weight)
    ...
    def forward(self, adj, users=None, train=False):
        ...
        if train:
            return final_user_e, final_item_e, mm_embeds, content_embeds
