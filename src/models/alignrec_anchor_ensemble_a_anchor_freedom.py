...
try:
    from .alignrec_anchor_1013 import ALIGNREC_ANCHOR_1013
    from .freedom import FREEDOM
except Exception:
    # 상대 임포트가 환경에 따라 실패할 수 있으므로 절대 임포트 백업
    from models.alignrec_anchor_1013 import ALIGNREC_ANCHOR_1013
    from models.freedom import FREEDOM

import sys, platform, torch, numpy as np, traceback, inspect, torch.nn as _nn

_MODEL_REGISTRY = {
    "ALIGNREC_ANCHOR_1013": ALIGNREC_ANCHOR_1013,
    "FREEDOM": FREEDOM,
}


class ALIGNREC_ANCHOR_ENSEMBLE_A_ANCHOR_FREEDOM(GeneralRecommender):
    """
    ...
    def __init__(self, config, dataset):
        ...
        self.model_name1 = str(config['agg_model1'] if 'agg_model1' in config else "ALIGNREC_ANCHOR_1013")
        self.model_name2 = str(config['agg_model2'] if 'agg_model2' in config else "FREEDOM")
        # ...
        ...
        print("[DEBUG] PyTorch:", torch.__version__, "| CUDA available:", torch.cuda.is_available(), "| Device:", self.device)
        print("[DEBUG] Python:", sys.version.split()[0], "| OS:", platform.platform())

        def _cfg_get(cfg, key, default=None):
            try:
                return cfg[key]           # dict-like (__getitem__)
            except Exception:
                try:
                    return getattr(cfg, key)  # attr-like
                except Exception:
                    return default

        def _cfg_set(cfg, key, value):
            try:
                cfg[key] = value          # dict-like
                return
            except Exception:
                try:
                    setattr(cfg, key, value)  # attr-like
                except Exception:
                    pass

        # 기본값: embedding_size -> 없으면 64
        emb_size = _cfg_get(config, 'embedding_size', 64)
        feat_dim = _cfg_get(config, 'feat_embed_dim', None)
        if feat_dim is None:
            _cfg_set(config, 'feat_embed_dim', int(emb_size))

        # (선택) 디버그 출력
        try:
            print(f"[FIX] feat_embed_dim set to {int(_cfg_get(config, 'feat_embed_dim'))} "
                f"(embedding_size={int(emb_size)})")
        except Exception:
            pass
