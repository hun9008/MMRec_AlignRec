...
class LGMREC_STORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(LGMREC_STORE, self).__init__(config, dataset)
    ...
    def forward(self, return_components=False):
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            mm_embs = all_embs - cge_embs
        else:
            all_embs = cge_embs
            mm_embs = torch.zeros_like(cge_embs)

        u_embs, i_embs = torch.split(all_embs, [self.n_users, self.n_items], dim=0)
        if return_components:
            _, id_item = torch.split(cge_embs, [self.n_users, self.n_items], dim=0)
            _, mm_item = torch.split(mm_embs, [self.n_users, self.n_items], dim=0)
            return u_embs, i_embs, id_item, mm_item
    ...
    def full_sort_predict(self, interaction):
        user = interaction[0]
        user_embs, item_embs, id_item, mm_item = self.forward(return_components=True)

        save_dir = os.path.join("saved_emb", self.dataset_name, "LGMRec")
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "final_item.npy"), item_embs.detach().cpu().numpy())
