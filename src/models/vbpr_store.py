...
class VBPR_STORE(GeneralRecommender):
    r"""BPR is a basic matrix factorization model that be trained in the pairwise way.
    """
    def __init__(self, config, dataloader):
        super(VBPR_STORE, self).__init__(config, dataloader)
    ...
    def full_sort_predict(self, interaction):
        ...
        final_item = item_embeddings
        id_item = self.i_embedding
        mm_item = self.item_linear(self.item_raw_features)

        save_dir = os.path.join("saved_emb", self.dataset_name, "VBPR")
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "final_item.npy"), final_item.detach().cpu().numpy())
