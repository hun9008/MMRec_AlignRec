...
class ALIGNREC_ANCHOR_ADD_PROJ(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_ADD_PROJ, self).__init__(config, dataset)
        self.sparse = True
        ...
        # [ADDED] projection 기반 residual을 최종 item 임베딩에 주입할지 여부 (기본 True)
        self.use_align_proj_out = config['use_align_proj_out'] if 'use_align_proj_out' in config else True
        # [ADDED] residual 스케일(learnable). 초기 영향도를 너무 세게 주지 않도록 0.3 정도 권장
        init_gamma = float(config['align_proj_gamma_init']) if 'align_proj_gamma_init' in config else 0.3
        self.gamma_align = nn.Parameter(torch.tensor(init_gamma, dtype=torch.float32))
        ...
        # [ADDED] projection 출력들을 합쳐서 D로 줄이는 선형 헤드 (id/mm/t/v → concat → D)
        self.item_align_fuse = nn.Linear(self.embedding_dim * 4, self.embedding_dim, bias=False)
        nn.init.xavier_uniform_(self.item_align_fuse.weight)
    ...
    def forward(self, adj, users=None, train=False):
        ...
        # 최종 반환용 split (user/item)
        all_embeddings_users, all_embeddings_items = torch.split(all_embeds, [self.n_users, self.n_items], dim=0)

        # [ADDED] ====== projection 결과를 최종 item 임베딩에 residual로 주입 ======
        if self.use_align_proj_out:
            # align-space projections (아이템만)
            content_embeds_user, content_embeds_items = torch.split(content_embeds, [self.n_users, self.n_items], dim=0)
            side_embeds_users, side_embeds_items = torch.split(mm_embeds, [self.n_users, self.n_items], dim=0)

            h_id_i_fusion = self.W_id_i(content_embeds_items)     # (I, D)
            h_mm_i_fusion = self.W_mm_i(side_embeds_items)        # (I, D)
            h_t_fusion = self.W_t(self.t_feat)                    # (I, D)  raw text → align
            h_v_fusion = self.W_v(self.v_feat)                    # (I, D)  raw vision → align

            cat = torch.cat([h_id_i_fusion, h_mm_i_fusion, h_t_fusion, h_v_fusion], dim=-1)  # (I, 4D)
            delta_item = self.item_align_fuse(cat)  # (I, D)

            # residual add (학습 가능한 스케일)
            all_embeddings_items = all_embeddings_items + self.gamma_align * delta_item
        # [ADDED] ===============================================================
