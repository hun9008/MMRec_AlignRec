...
class ALIGNREC_ANCHOR_ADD_PROJ_EMB_0922(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_ADD_PROJ_EMB_0922, self).__init__(config, dataset)
        self.sparse = True
        ...
        self.use_proj_residual = True  # 켜고 끌 스위치
        # 각 projection 성분 가중치 (학습 가능)
        self.proj_w_id = nn.Parameter(torch.tensor(1.0))
        self.proj_w_mm = nn.Parameter(torch.tensor(1.0))
        self.proj_w_t  = nn.Parameter(torch.tensor(1.0))
        self.proj_w_v  = nn.Parameter(torch.tensor(1.0))
        # 전체 residual 크기 조절(학습 가능)
        self.proj_alpha = nn.Parameter(torch.tensor(0.5))
        # 안정화용 레이어노름(선택)
        self.proj_ln = nn.LayerNorm(self.embedding_dim)
    ...
    def forward(self, adj, users=None, train=False):
        ...
        id_items = content_embeds[self.n_users:]   # (n_items, d)
        mm_items = mm_embeds[self.n_users:]        # (n_items, d)

        id_proj = self.W_id_i(id_items)            # (n_items, d)
        mm_proj = self.W_mm_i(mm_items)            # (n_items, d)
        t_proj  = self.W_t(self.t_feat)            # (n_items, d)  # raw text feat -> proj
        v_proj  = self.W_v(self.v_feat)            # (n_items, d)  # raw vision feat -> proj

        # 가중합 (학습 가능 가중치 + 전체 스케일)
        proj_sum = (
            self.proj_w_id * id_proj +
            self.proj_w_mm * mm_proj +
            self.proj_w_t  * t_proj  +
            self.proj_w_v  * v_proj
        )
        # (선택) 안정화
        proj_sum = self.proj_ln(proj_sum)

        all_embeddings_items = all_embeddings_items + self.proj_alpha * proj_sum
