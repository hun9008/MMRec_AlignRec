...
class BM3_STORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(BM3_STORE, self).__init__(config, dataset)
    ...
    def full_sort_predict(self, interaction):
        ...
        mm_items = []
        if self.t_feat is not None:
            mm_items.append(self.predictor(self.text_trs(self.text_embedding.weight)))
        if self.v_feat is not None:
            mm_items.append(self.predictor(self.image_trs(self.image_embedding.weight)))
        if len(mm_items) > 0:
            mm_item = torch.mean(torch.stack(mm_items), dim=0)
        else:
            mm_item = torch.zeros_like(i_online)

        save_dir = os.path.join("saved_emb", self.dataset_name, "BM3")
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "final_item.npy"), i_online.detach().cpu().numpy())
        np.save(os.path.join(save_dir, "id_item.npy"), i_online_ori.detach().cpu().numpy())
