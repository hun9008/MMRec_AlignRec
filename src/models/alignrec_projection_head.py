...
class ALIGNREC_PROJECTION_HEAD(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_PROJECTION_HEAD, self).__init__(config, dataset)
        self.sparse = True
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        itme_cl_loss = self.InfoNCE(h_id_i_fusion, h_mm_i_fusion, 0.2)
        
        cl_mm = self.InfoNCE(h_mm_i_fusion, h_id_i_fusion, 0.2) + self.InfoNCE(h_mm_i_fusion, h_v_fusion, 0.2) + self.InfoNCE(h_mm_i_fusion, h_t_fusion, 0.2)

        projection_loss = itme_cl_loss + cl_mm
        ...
        if not_train_ui:
            return batch_emb_loss + batch_reg_loss + self.cl_loss * cl_loss + self.sim_weight * ii_sim_loss + self.p_weight * projection_loss 
        return batch_mf_loss + batch_emb_loss + batch_reg_loss + self.cl_loss * cl_loss + self.sim_weight * ii_sim_loss + self.p_weight * projection_loss
