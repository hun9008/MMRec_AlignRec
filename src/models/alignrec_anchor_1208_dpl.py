...
from common.dpl_module import DPLFusion

class ALIGNREC_ANCHOR_1208_DPL(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_1208_DPL, self).__init__(config, dataset)
        self.sparse = True
        ...
        # DPL: vision/text 멀티모달 융합 모듈
        self.dpl = DPLFusion(self.embedding_dim)
        # mm_embeds에 DPL 출력(E_M)을 얼마나 섞을지 조절하는 스칼라 (learnable)
        self.dpl_gamma = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, adj, users=None, train=False):
        ...
        # ===== DPL: t_embeds, v_embeds를 이용해 멀티모달 표현 E_M 생성 =====
        # (n_users + n_items, d) 형태 그대로 사용
        E_common = self.dpl.extract_common(t_embeds, v_embeds)
        ...
        if self.use_recovered_mm_for_final:
            ...
            # 원래 쓰던 복원된 MM(id-space)도 추가하고 싶으면 아래 유지, 아니면 주석 처리
            # final_item_e = final_item_e + self.recover_lambda * mm_rec_i
        else:
            # v, t를 직접 더하지 않고, DPL로 융합된 enhanced_mm_embeds만 사용
            all_embeds = content_embeds + mm_embeds + t_embeds + v_embeds
            final_user_e, final_item_e = torch.split(
                all_embeds, [self.n_users, self.n_items], dim=0
            )
