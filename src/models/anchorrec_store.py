...
class ANCHORREC_STORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ANCHORREC_STORE, self).__init__(config, dataset)
        self.sparse = True
    ...
    def full_sort_predict(self, interaction):
        ...
        mm_item = mm_embeds[self.n_users:]
        id_item = content_embeds[self.n_users:]
        t_item = t_embeds[self.n_users:]
        v_item = v_embeds[self.n_users:]

        save_dir = os.path.join("saved_emb", self.dataset_name, "AnchorRec")
        ...
        np.save(os.path.join(save_dir, "text_item.npy"), t_item.detach().cpu().numpy())
        np.save(os.path.join(save_dir, "vision_item.npy"), v_item.detach().cpu().numpy())
