...
class ALIGNREC_REC_OVERLAP_0915(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_REC_OVERLAP_0915, self).__init__(config, dataset)
        self.sparse = True
    ...
    def full_sort_predict(self, interaction, K_list=(20,), exclude_seen=False):
        ...
        - item_emb_final_alignrec.npy         : (n_items, d)   # 최종(final) 아이템 임베딩
        - item_emb_id_items.npy               : (n_items, d)   # id(view) 아이템 임베딩
        - item_emb_mm_items.npy               : (n_items, d)   # mm(view) 아이템 임베딩
        - item_emb_id_plus_mm.npy             : (n_items, d)   # id + mm (가중합 실험시 여기서 바꾸면 됨)
        - rec_top{K}_{alias}.npy              : (n_users, K), alias in {final, id, mm, idmm}
        """
        user = interaction[0]
        save_dir = "saved_emb/AlignRec_output"
        os.makedirs(save_dir, exist_ok=True)

        with torch.no_grad():
            # NOTE: train=True 로 호출해 mm_embeds / content_embeds까지 받아옴
            if self.use_hist_decoder:
                U_all, V_all, mm_all, id_all, _ = self.forward(self.norm_adj, users=None, train=True)
            else:
                U_all, V_all, mm_all, id_all = self.forward(self.norm_adj, users=None, train=True)

            # ---- 유저/아이템 분리 ----
            U = U_all                               # (n_users, d)
            V_final = V_all                         # (n_items, d)  최종 아이템 임베딩
            id_items = id_all[self.n_users:]        # (n_items, d)  content(view) 중 아이템 파트
            mm_items = mm_all[self.n_users:]        # (n_items, d)
