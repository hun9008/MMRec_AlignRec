...
class DAMRS_STORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(DAMRS_STORE, self).__init__(config, dataset)
    ...
    def full_sort_predict(self, interaction):
        ...
        mm_item = (h_v + h_t + h_s) / 3.0
        all_item_e = item_embeddings + mm_item

        save_dir = os.path.join("saved_emb", self.dataset_name, "DAMRS")
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "final_item.npy"), all_item_e.detach().cpu().numpy())
        np.save(os.path.join(save_dir, "id_item.npy"), item_embeddings.detach().cpu().numpy())
