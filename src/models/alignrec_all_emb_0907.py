...
class ALIGNREC_ALL_EMB_0907(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ALL_EMB_0907, self).__init__(config, dataset)
        self.sparse = True
    ...
    def full_sort_predict(self, interaction):
        ...
        # train=True 경로로 호출해 모든 view를 받음
        ua_embeddings, ia_embeddings, mm_embeds, content_embeds = self.forward(self.norm_adj, train=True)

        save_dir = "saved_emb/epoch_last"  # 필요시 config로 받으세요
        os.makedirs(save_dir, exist_ok=True)

        # ---------- 저장 ----------
        # 1) RAW ID (LightGCN UI-view에서 fusion 전: 아이템 파트)
        h_id_raw = content_embeds[self.n_users:]                      # (n_items, d)
        np.save(os.path.join(save_dir, "item_emb_raw_id.npy"),
                h_id_raw.detach().cpu().numpy())

        # 2) MM OUT (alignment space 밖, projection/fusion 전: 아이템 파트)
        h_mm_out = mm_embeds[self.n_users:]                           # (n_items, d)
        np.save(os.path.join(save_dir, "item_emb_mm_out.npy"),
                h_mm_out.detach().cpu().numpy())

        # 3) FINAL (ALIGNREC 최종 아이템 임베딩: fusion 후)
        final_item = ia_embeddings                                    # (n_items, d)
        np.save(os.path.join(save_dir, "item_emb_final_alignrec.npy"),
                final_item.detach().cpu().numpy())

        # (옵션) 원본 멀티모달 feature도 보관
        if getattr(self, "mm_feat", None) is not None:
            np.save(os.path.join(save_dir, "item_feat_raw_mm.npy"),
                    self.mm_feat.detach().cpu().float().numpy())

        # 기존 점수 계산(원 코드 유지 필요시)
        # eval 경로와 동일하게 사용자 임베딩 하나 뽑아 전체 아이템과 내적
        # 여기서는 방금 만든 ua_embeddings, final_item 재사용
        u_embeddings = ua_embeddings[user]
        scores = torch.matmul(u_embeddings, final_item.transpose(0, 1))
        return scores
