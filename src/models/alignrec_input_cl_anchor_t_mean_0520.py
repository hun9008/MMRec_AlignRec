...
class ALIGNREC_INPUT_CL_ANCHOR_T_MEAN_0520(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_INPUT_CL_ANCHOR_T_MEAN_0520, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, users=None, train=False):
        ...
        if self.side_emb_div!=0:
            all_embeds = (content_norm + t_norm) / self.side_emb_div
        else: 
            all_embeds = (content_embeds + t_embeds) / 3
        all_embeddings_users, all_embeddings_items = torch.split(all_embeds, [self.n_users, self.n_items], dim=0)
