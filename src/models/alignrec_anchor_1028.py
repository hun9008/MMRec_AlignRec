...
class ALIGNREC_ANCHOR_1028(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_1028, self).__init__(config, dataset)
        self.sparse = True
        ...
        # 최종 임베딩에 복원된 MM만 더할지 제어(필요시 config로 뺄 수 있음)
    ...
    def forward(self, adj, users=None, train=False):
        ...
        mm_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_v(mm_feats))  # encoder output-like
        ...
        # ----- Graph propagation in item-item views -----
        if self.sparse:
        ...
        # (t_embeds, v_embeds도 필요시 split 가능)
        ...
        mm_rec_i = self.D_mm2id(h_mm_i_fusion)

        # ----- Final embeddings -----
        if self.use_recovered_mm_for_final:
            # 최종 아이템: ID + MM + recovered(MM)
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        if self.ui_cosine_loss:
            ...
            try:
                ...
                if isinstance(self.ui_cosine_loss_weight, torch.Tensor):
                    ...
                    raise TypeError(f"ui_cosine_loss_weight must be a float-like scalar, got {type(self.ui_cosine_loss_weight)}")

            # 2) user는 기준점만 제공 (no grad)
            u_anchor = u_g_embeddings.detach()

            # 3) batch-wise alignment (스칼라로 평균)
        ...
        pos_tt_batch_sim_mat = build_sim(self.t_feat[pos_items])  # text 원피처
        pos_vv_batch_sim_mat = build_sim(self.v_feat[pos_items])  # vision 원피처
        ...
        if self.use_bce:
            ...
            ii_t_sim_loss = self.sim_sigmoid_loss(t_embeds_items[pos_items], pos_tt_batch_sim_mat)
            ii_v_sim_loss = self.sim_sigmoid_loss(v_embeds_items[pos_items], pos_vv_batch_sim_mat)
        else:
            ii_sim_loss = self.sim_loss(side_embeds_items[pos_items], pos_ii_batch_sim_mat)
            ii_t_sim_loss = self.sim_loss(t_embeds_items[pos_items], pos_tt_batch_sim_mat)
            ii_v_sim_loss = self.sim_loss(v_embeds_items[pos_items], pos_vv_batch_sim_mat)
        ...
        if not_train_ui:
            return batch_emb_loss + cl_loss * self.cl_loss + self.sim_weight * ii_sim_loss
        return batch_mf_loss + batch_emb_loss + cl_loss * self.cl_loss + self.sim_weight * ii_sim_loss
