...
class ALIGNREC_INPUT_CL_ANCHOR_MMTV_IISIM_DIV_0907(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_INPUT_CL_ANCHOR_MMTV_IISIM_DIV_0907, self).__init__(config, dataset)
        self.sparse = True
    ...
    def full_sort_predict(self, interaction):
        ...
        os.makedirs("saved_emb", exist_ok=True)

        # id / mm (item 파트)
        h_id_i = content_embeds[self.n_users:]      # h_id
        h_mm_i = mm_embeds[self.n_users:]           # h_mm
        np.save("saved_emb/item_emb_alignrec_anchor_id.npy", h_id_i.detach().cpu().numpy())
        np.save("saved_emb/item_emb_alignrec_anchor_mm.npy", h_mm_i.detach().cpu().numpy())

        # t / v (item 파트)
        h_t = t_embeds[self.n_users:]
        h_v = v_embeds[self.n_users:]
        np.save("saved_emb/item_emb_alignrec_anchor_text.npy",   h_t.detach().cpu().numpy())
        np.save("saved_emb/item_emb_alignrec_anchor_vision.npy", h_v.detach().cpu().numpy())

        # 기존 projection 평균도 유지
        with torch.no_grad():
            h_id_i_proj = self.W_id_i(h_id_i)
            h_mm_i_proj = self.W_mm_i(h_mm_i)
            h_proj_aligned = (h_id_i_proj + h_mm_i_proj) / 2
            np.save("saved_emb/item_emb_alignrec_anchor_projection_aligned.npy",
                    h_proj_aligned.cpu().numpy())
