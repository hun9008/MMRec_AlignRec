...
class ANCHORREC_ORIGIN_PROJ_ALIGN(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ANCHORREC_ORIGIN_PROJ_ALIGN, self).__init__(config, dataset)
        self.sparse = True
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        # Use the original-space MM embedding as a fixed anchor, while keeping
        # ID and mixup views in the projection space.
        mm_anchor = side_embeds_items[pos_items]
        cl_id_origin = self.InfoNCE(
            mm_anchor,
            content_embeds_items[pos_items],
            self.tau
        )
        cl_id_proj = self.InfoNCE(
        ...
        aal_loss = cl_id_origin + cl_id_proj + cl_mixup
