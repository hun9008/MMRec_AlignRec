...
class ALIGNREC_ANCHOR_1201(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALIGNREC_ANCHOR_1201, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, users=None, train=False):
        ...
        # if self.sparse:
        #     for _ in range(self.n_layers):
        #         mm_item_embeds = torch.sparse.mm(self.mm_original_adj, self.mm_feat)
        # else:
        #     for _ in range(self.n_layers):
        #         mm_item_embeds = torch.mm(self.mm_original_adj, self.mm_feat)
        # mm_user_embeds = torch.sparse.mm(self.R, mm_item_embeds)
        # mm_embeds = torch.cat([mm_user_embeds, mm_item_embeds], dim=0)
        ...
        # if self.sparse:
        #     for _ in range(self.n_layers):
        #         t_item_embeds = torch.sparse.mm(self.t_original_adj, self.t_feat)
        # else:
        #     for _ in range(self.n_layers):
        #         t_item_embeds = torch.mm(self.t_original_adj, self.t_feat)
        # t_user_embeds = torch.sparse.mm(self.R, t_item_embeds)
        # t_embeds = torch.cat([t_user_embeds, t_item_embeds], dim=0)
        ...
        # if self.sparse:
        #     for _ in range(self.n_layers):
        #         v_item_embeds = torch.sparse.mm(self.v_original_adj, self.v_feat)
        # else:
        #     for _ in range(self.n_layers):
        #         v_item_embeds = torch.mm(self.v_original_adj, self.v_feat)
        # v_user_embeds = torch.sparse.mm(self.R, v_item_embeds)
        # v_embeds = torch.cat([v_user_embeds, v_item_embeds], dim=0)
        ...
        h_t_i_fusion = self.W_t(self.t_feat)                  # (n_items, d)
        h_v_i_fusion = self.W_v(self.v_feat)                  # (n_items, d)

        # ----- Final embeddings for scoring -----
        if self.use_recovered_mm_for_final:
            # 최종 아이템: ID + MM + text + 복원된MM(가중)
        ...
        if train:
            if self.use_hist_decoder:
                ...
                # return final_user_e, final_item_e, h_mm_i_fusion, content_embeds, h_t_i_fusion, h_v_i_fusion, hist_hid[:, -1, :]
            return final_user_e, final_item_e, mm_embeds, content_embeds, t_embeds, v_embeds
            # return final_user_e, final_item_e, h_mm_i_fusion, content_embeds, h_t_i_fusion, h_v_i_fusion
