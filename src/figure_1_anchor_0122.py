...
def visualize_alignment_3views(
    ...
    focus_item_idx=993,
    zoom_in_scale=0.35,       # ✅ 아래 zoom-in 반경을 더 줄여 확대 (작을수록 더 확대)
    zoom_height_ratio=3.0,    # ✅ 아래 subplot 높이 비율
):
    # ===== 1. tensor -> numpy =====
    id_np = id_embeds.detach().cpu().numpy()
    content_np = content_embeds_items.detach().cpu().numpy()
    ...
    # ✅ zoom_mask도 zoom-in 반경 기준으로 계산 (아래에서 선/포커스 강조가 정확해짐)
    zoom_mask = (
    ...
    fig, (ax_main, ax_zoom) = plt.subplots(
        2, 1,
        figsize=(10, 10),  # ✅ 아래 zoom-in이 더 커 보이도록 figure 높이 증가
        gridspec_kw={'height_ratios': [1.0, zoom_height_ratio], 'hspace': 0.06}
    )
    ...
    ax_zoom.scatter(
        content_2d[:, 0], content_2d[:, 1],
        alpha=base_alpha, marker='s', s=60, color=scat_mm.get_facecolor()[0], zorder=2
    )
    ax_zoom.scatter(
        id_2d[:, 0], id_2d[:, 1],
        alpha=base_alpha, marker='o', s=60, color=scat_id.get_facecolor()[0], zorder=3
    )
    ...
    if focus_item_idx is not None:
        for i in range(sample_size):
            if not zoom_mask[i]:
                continue
            ax_zoom.annotate(
                f"{indices[i]}",
                (final_2d[i, 0], final_2d[i, 1]),
    ...
    # ✅ 2) 아래 zoom-in은 zoom_r_zoom으로 더 확대되게 설정
...
def run_tsne_visualization(model_name, checkpoint_path, lines_to_draw):
    ...
    #     filename = f"{scale}_tsne_{model_name}_anchor_focus{focus_idx:02d}_0202.png"
    ...
    for focus_idx in focus_idx_list:
        filename = f"{scale}_tsne_{model_name}_anchor_focus{focus_idx:02d}_0206.png"
        visualize_alignment_3views(
            id_items,
            content_items,
        final_items,
