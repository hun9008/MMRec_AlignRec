...
def compute_calinski_for_embedding(
    emb_np,
    name="embedding",
    n_clusters=400,
    sample_size=None,
    ...
    sample_size: CH score 계산에 사용할 샘플 수
    ...
    score = calinski_harabasz_score(emb_eval, cluster_labels)

    space_type = "t-SNE 2D projection" if use_tsne_projection else "original embedding space"
    print(f"[{name}] Calinski-Harabasz score on {space_type}: {score:.6f}")
...
def compute_all_calinski_scores(
    id_items,
    content_items,
    final_items,
    n_clusters=10,
    ...
    print("=" * 80)
    print("Calinski-Harabasz Score Calculation")
    print("NOTE: CH score should be computed in the ORIGINAL embedding space.")
    print("NOTE: Higher is better.")
    print("=" * 80)
    ...
    # 1. ID embedding
    results.append(
        compute_calinski_for_embedding(
            emb_np=id_np,
            name="ID",
            n_clusters=n_clusters,
...
def print_results_table(results):
    print("\n" + "=" * 80)
    print(f"{'Name':<12} {'Space':<28} {'#Samples':<10} {'#Clusters':<10} {'CH Score':<15}")
    print("-" * 80)
    for r in results:
        print(
            f"{r['name']:<12} "
...
def run_calinski_evaluation(
    ...
    # ----- 3) CH score 계산 -----
    results = compute_all_calinski_scores(
        id_items=id_items,
...
if __name__ == '__main__':
    ...
    for model_name, ckpt_path in models:
        run_calinski_evaluation(
            model_name=model_name,
            checkpoint_path=ckpt_path,
            n_clusters=400,       # 필요시 조절
            sample_size=70000,   # 전체가 너무 크면 일부 샘플만 평가
        )