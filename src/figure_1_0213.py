...
def visualize_alignment_3views(
    ...
    focus_item_idx=993,
    zoom_in_scale=0.35,       # ✅ 오른쪽 zoom-in 반경을 더 줄여 확대 (작을수록 더 확대)
    zoom_height_ratio=3.0,    # ✅ 오른쪽 subplot 너비 비율 (이름은 호환성 유지)
    zoom_legend_fontsize=20,
    show_zoom_legend=True,
    main_show_full_legend=False,
    main_legend_fontsize=20,
    zoom_square_box=False,
):
    # ===== 1. tensor -> numpy =====
    id_np = id_embeds.detach().cpu().numpy()
    content_np = content_embeds_items.detach().cpu().numpy()
    ...
    # ✅ 메인 원 반경 / 오른쪽 zoom-in 반경 분리
    zoom_r_main = zoom_r
    zoom_r_zoom = np.clip(zoom_r_main * zoom_in_scale, 0.6, zoom_r_main)

    print(
        f"center_x : {center_x}, center_y : {center_y}, "
        f"zoom_r_main : {zoom_r_main}, zoom_r_zoom : {zoom_r_zoom}"
    )

    # ✅ zoom_mask도 zoom-in 반경 기준으로 계산
    zoom_mask = (
        (final_2d[:, 0] >= center_x - zoom_r_zoom) &
        (final_2d[:, 0] <= center_x + zoom_r_zoom) &
        (final_2d[:, 1] >= center_y - zoom_r_zoom) &
        (final_2d[:, 1] <= center_y + zoom_r_zoom)
    )
    ...
    fig, (ax_main, ax_zoom) = plt.subplots(
        1, 2,
        figsize=(14, 5),  # ✅ 오른쪽 zoom-in이 더 크게 보이도록 figure 너비 증가
        gridspec_kw={'width_ratios': [1.0, zoom_height_ratio], 'wspace': 0.06}
    )
    ...
    scat_mm = ax_main.scatter(
        content_2d[:, 0], content_2d[:, 1],
        label='MM', alpha=0.7, marker='s', s=2.5
    )
    scat_id = ax_main.scatter(
        id_2d[:, 0], id_2d[:, 1],
        label='ID', alpha=0.7, marker='o', s=2.5
    )
    # Final은 메인(전체) 시각화에서 숨김: ID/MM만 보여줌
    final_color = '#2ca02c'
    ...
    # ===== 6-2. 오른쪽 zoom 축 =====
    # ax_zoom.set_title("Zoom-in of highlighted region", fontsize=12)

    # 1) 전체 점들을 아주 연하게 그림
    base_alpha = 0.5
    ax_zoom.scatter(
        content_2d[:, 0], content_2d[:, 1],
        alpha=base_alpha, marker='s', s=60, color=scat_mm.get_facecolor()[0]
    )
    ax_zoom.scatter(
        id_2d[:, 0], id_2d[:, 1],
        alpha=base_alpha, marker='o', s=60, color=scat_id.get_facecolor()[0]
    )
    scat_final = ax_zoom.scatter(
        final_2d[:, 0], final_2d[:, 1],
        alpha=base_alpha, marker='*', s=80, color=final_color, label='Final'
    )

    # ✅ 2) 오른쪽 zoom-in은 zoom_r_zoom으로 더 확대되게 설정
    ax_zoom.set_xlim(center_x - zoom_r_zoom, center_x + zoom_r_zoom)
    ax_zoom.set_ylim(center_y - zoom_r_zoom, center_y + zoom_r_zoom)
    if zoom_square_box:
        # 오른쪽 패널 박스를 정사각형으로 고정 (가로=세로)
    ...
    # 4) 포커스 아이템 점 3개(ID/MM/Final)를 진하게 찍기
    ...
    legend_id = Line2D([0], [0], marker='o', linestyle='None', color='none',
                       markerfacecolor=scat_id.get_facecolor()[0], markeredgecolor='none',
                       markersize=20, label='ID')
    legend_mm = Line2D([0], [0], marker='s', linestyle='None', color='none',
                       markerfacecolor=scat_mm.get_facecolor()[0], markeredgecolor='none',
                       markersize=20, label='MM')
    legend_final = Line2D([0], [0], marker='*', linestyle='None', color='none',
                          markerfacecolor=final_color, markeredgecolor='none',
                          markersize=24, label='Final')
    if main_show_full_legend:
        ax_main.legend(
            handles=[legend_id, legend_mm, legend_final, line_id_final, line_mm_final],
            fontsize=main_legend_fontsize,
            loc='lower left',
        )
    else:
        ax_main.legend(
            handles=[scat_id, scat_mm],
            fontsize=main_legend_fontsize,
            markerscale=8,
            loc='lower left'
        )
...
def run_tsne_visualization(model_name, checkpoint_path, lines_to_draw):
    ...
    # ----- 3) t-SNE 그리기 -----
    scale = 0.15
    # for focus_idx in range(61, 121):
    #     filename = f"{scale}_tsne_{model_name}_focus{focus_idx:02d}_0202.png"
    ...
    #         filename=filename,
    #         focus_item_idx=focus_idx,
    #         zoom_in_scale=scale,
    #         zoom_height_ratio=3.0,
    #     )
    # focus_idx = 84
    # filename = f"{scale}_tsne_{model_name}_focus{focus_idx:02d}_0202.png"
    # visualize_alignment_3views(
    ...
    #     zoom_height_ratio=3.0,
    # )
    # focus_idx_list = [1,3, 7,19,49,81,84]
    focus_idx_list = [19]

    # 좌:우 패널 비율 케이스
    # 현재 visualize_alignment_3views의 zoom_height_ratio는 "오른쪽/왼쪽" 비율이므로
    # 2:1(좌:우)은 0.5로 전달
    panel_ratio_cases = [
        # (ratio_tag, right_to_left_ratio, zoom_legend_fontsize, show_zoom_legend, main_show_full_legend, main_legend_fontsize, zoom_square_box)
        ("1_1", 1.0, 18, True,  False, 20, False),  # 좌:우 = 1:1
        ("2_1", 0.5, 20, False, True,  18, False),  # 좌:우 = 2:1, legend는 왼쪽에만(글자 약간 축소)
        ("1_2", 2.0, 20, True,  False, 20, True),   # 좌:우 = 1:2, 오른쪽 패널 정사각형
    ]

    for focus_idx in focus_idx_list:
        for (
            ratio_tag,
            right_to_left_ratio,
            zoom_legend_fontsize,
            show_zoom_legend,
            main_show_full_legend,
            main_legend_fontsize,
            zoom_square_box,
        ) in panel_ratio_cases:
            filename = (
                f"{scale}_tsne_{model_name}_focus{focus_idx:02d}_0213_ratio_{ratio_tag}.png"
            )
            visualize_alignment_3views(
                id_items,
                content_items,
                ...
                focus_item_idx=focus_idx,
                zoom_in_scale=scale,
                zoom_height_ratio=right_to_left_ratio,
                zoom_legend_fontsize=zoom_legend_fontsize,
                show_zoom_legend=show_zoom_legend,
                main_show_full_legend=main_show_full_legend,
                main_legend_fontsize=main_legend_fontsize,
                zoom_square_box=zoom_square_box,
            )
