...
def _tsne_2d(mm_items, t_items, v_items, indices, random_state=42):
    ...
    v_2d = tsne_result[sample_size * 2:]
    return mm_2d, t_2d, v_2d
...
def visualize_pretrain_vs_origin(pretrain, origin, sample_size, title, filename, seed=42):
    ...
    pre_mm_2d, pre_t_2d, pre_v_2d = _tsne_2d(
        pretrain["MM"], pretrain["Text"], pretrain["Vision"], indices, random_state=seed
    )
    org_mm_2d, org_t_2d, org_v_2d = _tsne_2d(
        origin["MM"], origin["Text"], origin["Vision"], indices, random_state=seed
    )

    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    axes[0].set_title("Pre-trained embeddings (MM/Text/Vision)")
    axes[1].set_title("Origin space embeddings (MM/Text/Vision)")
...
def _extract_pretrain_embeddings(model):
    ...
    return {"MM": mm_pre, "Text": t_pre, "Vision": v_pre}
...
def _extract_origin_embeddings(model):
    ...
    return {"MM": mm_items, "Text": t_items, "Vision": v_items}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--title",
        type=str,
        default="t-SNE: pre-trained vs origin (MM/Text/Vision)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="tsne_pretrain_origin_0203.png",
    )
