...
def _tsne_2d(emb_dict, indices, random_state=42):
    keys = ["ID", "MM", "Text", "Vision"]
    arrays = [emb_dict[k].detach().cpu().numpy()[indices] for k in keys]
    ...
    for i, k in enumerate(keys):
        ...
        out[k] = tsne_result[start:end]
    return out


def visualize_origin_vs_projection(origin, proj, sample_size, title, filename, seed=42):
    ...
    origin_2d = _tsne_2d(origin, indices, random_state=seed)
    proj_2d = _tsne_2d(proj, indices, random_state=seed)
    ...
    axes[0].set_title("Origin space (before projection)")
    axes[1].set_title("Projection space (after projection)")
...
def main(args):
    ...
    visualize_origin_vs_projection(
        origin,
        proj,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--title",
        type=str,
        default="t-SNE: origin vs projection (ID/MM/Text/Vision)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="tsne_origin_vs_projection_0203.png",
    )
