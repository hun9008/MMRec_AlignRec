...
INTER_PATH    = "./data/baby/baby.inter"   # ✅ 기본 interactions 경로 (TSV)
...
# ------------- main -------------
def main():
    ap = argparse.ArgumentParser(description="Visualize top-k close neighbors for a fixed anchor i (with 2-hop counts).")
    ...
    ap.add_argument("--out", type=str, default="ijkl_visualization_topkclose.png", help="출력 파일명")
    ...
    # ✅ interactions는 파라미터 없이 기본 경로 사용. 필요시만 끄도록 옵션 추가(기본 False)
    ap.add_argument("--no-interactions", action="store_true", help="2-hop 계산 끄기 (기본: 켜짐, 경로는 INTER_PATH)")
    args = ap.parse_args()
