...
def pick_common_anchor(
    ...
    if has_a and has_b:
        logging.info(f"요청 target {want_i} 사용 (두 모델 모두 top-{topk} OK)")
        return want_i

    if not has_a:
        logging.warning(f"AlignRec 결과에 target {want_i} 없음 또는 close 부족")
    if not has_b:
        logging.warning(f"Anchor 결과에 target {want_i} 없음 또는 close 부족")
    ...
    if candidates:
        ...
        logging.warning(f"→ 공통 target {chosen} 사용")
        return chosen

    raise ValueError("공통 target을 찾을 수 없습니다.")
...
def main():
    ...
    ap.add_argument("--target", type=int, required=True, help="시각화할 target i")
    ...
    anchor_i = pick_common_anchor(args.target, i2js_alignrec, i2js_anchor, args.topk)
    js_alignrec = i2js_alignrec[anchor_i][:args.topk]
    js_anchor   = i2js_anchor[anchor_i][:args.topk]

    logging.info(
        f"최종 target={anchor_i}, AlignRec close={js_alignrec}, Anchor close={js_anchor}"
    )
    ...
    if (not args.no_img_sim) and _HAS_TORCH and _HAS_TORCHVISION and _HAS_PIL_REQ:
        if setup_image_model(args.device):
            ...
            if anchor_img is not None:
                ...
                logging.warning("target 이미지 없음 → img cos 생략")
        else:
    ...
    if not ext:
        ext = ".png"
    out_png = f"{base}_{args.data}_{anchor_i}{ext}"
