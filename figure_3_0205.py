import os, json, html, argparse, re
import numpy as np
import pandas as pd
...
def _split_top_level_dicts(text: str) -> List[str]:
    objs = []
    buf = []
    depth = 0
    in_str = False
...
def read_metadata_all(path_json: str) -> Dict[str, dict]:
    ...
    if not data:
        objs = _split_top_level_dicts(text)
        for obj in objs:
            s = obj.strip()
            if not s:
                continue
...
def has_topk(i: int, i2js: Dict[int, list], topk: int) -> bool:
    return (i in i2js) and (len(i2js[i]) >= topk)
...
def embed_text_bert(texts: List[str], max_len: int = 128) -> Optional[np.ndarray]:
    ...
    for i in range(0, len(texts), bs):
        ...
        with torch.no_grad():
            out = _TXT_MODEL(**enc).last_hidden_state
...
def visualize_alignrec_vs_anchor(
    ...
    if text_a:
        ax_a.text(
            0.02, -0.20, wrap_text(text_a, width=40, max_lines=8),
            transform=ax_a.transAxes, va="top", ha="left",
            fontsize=7, bbox=dict(boxstyle="round", facecolor="white", alpha=0.9),
        )
    ...
    for idx in range(topk):
        ...
        _, text, img = get_item_text_and_img(item_id, id2asin, meta)

        ax = fig.add_subplot(gs[0, idx + 1])
        ax.axis("off")

        ax.text(
            0.5, TITLE_Y, f"[Item j#{idx+1} ID] : {item_id}",
        ...
        if text:
            ax.text(
                0.02, TEXT_Y, wrap_text(text, width=40, max_lines=6),
                transform=ax.transAxes, va="top", ha="left",
                fontsize=6.5, bbox=dict(boxstyle="round", facecolor="white", alpha=0.9),
            )
    ...
    for idx in range(topk):
        ...
        ax.text(
            0.5, TITLE_Y + 0.05, f"[Item j#{idx+1} ID] : {item_id}",
            transform=ax.transAxes, ha="center", va="bottom",
            fontsize=8.5, fontweight="bold",
        )
...
def run_one_target(
    ...
    # all 모드: 둘 다 topk 없으면 skip
    if args.all:
        if not (has_topk(target_i, i2js_alignrec, args.topk) and has_topk(target_i, i2js_anchor, args.topk)):
            return False
        anchor_i = target_i
    else:
        # single 모드: 기존처럼 target 없으면 에러(원하면 여기만 fallback 로직 넣어도 됨)
        if not (has_topk(target_i, i2js_alignrec, args.topk) and has_topk(target_i, i2js_anchor, args.topk)):
            raise ValueError(f"target {target_i} missing in one of results or close<topk")
        anchor_i = target_i
    ...
    out_png = f"{out_prefix}_{args.data}_{anchor_i}{out_ext}"
    visualize_alignrec_vs_anchor(
        anchor_i, js_alignrec, js_anchor,
        id2asin, meta, out_png,
...
def main():
    ap = argparse.ArgumentParser(
        description="Visualize AlignRec vs Anchor close items with shared target column"
    )
    ap.add_argument("--data", type=str, default=DEFAULT_DATASET,
                    help="데이터셋 이름 (./data/<name>)")
    ap.add_argument("--target", type=int, required=True, help="시각화할 target i (all 모드에서는 무시됨)")
    ap.add_argument("--topk", type=int, default=3, help="각 모델당 close item 개수")
    ap.add_argument("--out", type=str, default="./RQ3/top3_neighbor.png")
    ap.add_argument("--device", type=str, default="cuda:6")
    ap.add_argument("--no-img-sim", dest="no_img_sim", action="store_true")
    ap.add_argument("--no-txt-sim", dest="no_txt_sim", action="store_true")
    ...
    ap.add_argument("--all", action="store_true", default=False,
                    help="True면 item 개수만큼 target=1..N 반복 생성")

    args = ap.parse_args()

    data_dir = os.path.join("./data", args.data)
    mapping_csv = os.path.join(data_dir, "i_id_mapping.csv")
    metadata_json = os.path.join(data_dir, f"meta_{args.data}.json")
    inter_path = os.path.join(data_dir, f"{args.data}.inter")

    if not os.path.exists(mapping_csv):
        raise FileNotFoundError(mapping_csv)
    if not os.path.exists(metadata_json):
        raise FileNotFoundError(metadata_json)
    if not os.path.exists(ALIGNREC_RESULT):
        raise FileNotFoundError(ALIGNREC_RESULT)
    if not os.path.exists(ANCHOR_RESULT):
        raise FileNotFoundError(ANCHOR_RESULT)

    if not (_HAS_TORCH and torch.cuda.is_available()) or (not str(args.device).startswith("cuda")):
        args.device = "cpu"

    # out prefix/ext
    base, ext = os.path.splitext(args.out)
    out_ext = ext if ext else ".png"
    out_dir = os.path.join(os.path.dirname(args.out), args.data)
    ...
    if args.all:
        if not _HAS_TQDM:
            raise RuntimeError("tqdm가 필요합니다. `pip install tqdm` 후 실행하세요.")

        n_items = len(id2asin)  # 요청대로 row 수 기반
        for t in tqdm(range(1, n_items + 1), total=n_items, desc=f"Render {args.data}", unit="item"):
            try:
                run_one_target(
                    t, args, id2asin, meta,
                    i2js_alignrec, i2js_anchor,
                    inter_path, out_prefix, out_ext
                )
            except Exception:
                # all 모드에서는 조용히 스킵 (원하면 여기서 continue 전에 카운팅만 할 수도 있음)
                continue
    else:
        run_one_target(
            args.target, args, id2asin, meta,
            i2js_alignrec, i2js_anchor,
            inter_path, out_prefix, out_ext
        )
