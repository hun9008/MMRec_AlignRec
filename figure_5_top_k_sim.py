...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "For each item, find top-k similar items from model final item embeddings "
            "and save text/vision cosine similarities to txt files."
        )
    )
    ...
    parser.add_argument("--alignrec-final", default="saved_emb/AlignRec/final_item.npy")
    parser.add_argument("--anchorrec-final", default="saved_emb/AnchorRec/final_item.npy")
    ...
    parser.add_argument("--alignrec-out", default="figure_5_AlignRec_sim.txt")
    parser.add_argument("--anchorrec-out", default="figure_5_AnchorRec_sim.txt")
...
def topk_neighbors_by_cosine(emb: np.ndarray, topk: int, block_size: int) -> np.ndarray:
    ...
    for start in range(0, n_items, block_size):
        ...
        if topk >= n_items:
            block_neighbors = np.argsort(-sims, axis=1)[:, :topk]
        else:
            part = np.argpartition(-sims, kth=topk - 1, axis=1)[:, :topk]
            order = np.argsort(-np.take_along_axis(sims, part, axis=1), axis=1)
            block_neighbors = np.take_along_axis(part, order, axis=1)

        neighbors[start:end] = block_neighbors.astype(np.int32)
...
def main() -> None:
    ...
    save_similarity_file(
        "AlignRec",
        Path(args.alignrec_final),
        Path(args.alignrec_out),
        text_norm,
        vision_norm,
        args.topk,
        args.block_size,
    ...
    save_similarity_file(
        "AnchorRec",
        Path(args.anchorrec_final),
        Path(args.anchorrec_out),
        text_norm,
        vision_norm,
        args.topk,
        args.block_size,
